...
export default function ChatPanel({ messages, onSend, isLoading }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-panel border-l border-slate-200 dark:border-white/5">
      {/* Header */}
      ...
      {/* Settings Drawer */}
      {settingsOpen && (
        <div className="border-b border-slate-200 dark:border-white/5 bg-surface px-4 py-4 flex flex-col gap-4">
          {/* Provider tabs */}
          <div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-xs font-mono text-foreground placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500"
            />
          </div>
          ...
          <div>
            <p className="text-xs font-medium text-muted uppercase tracking-wider mb-2">
              {t('chat.graphsPerRowLabel')}: <span className="text-violet-500">{localCount}</span>
            </p>
            <input
              type="range"
              min={1}
          ...
          <div className="mt-8 text-center">
            {!hasKey ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-muted">{t('chat.noKeyHint')}</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-muted">{t('chat.examplePrompt')}</p>
              </div>
            )}
          </div>
        )}
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="px-4 py-3 border-t border-slate-200 dark:border-white/5 flex gap-2">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          disabled={isLoading}
          placeholder={isLoading ? t('chat.thinking') : t('chat.placeholder')}
          className="flex-1 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-foreground placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-violet-500 focus:border-violet-500 disabled:opacity-50"
        />
        <button
          type="submit"
